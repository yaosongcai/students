 ...
 * 类描述
 * <pre>
 *     多标签table(暂无提供get/set赋值属性的方法需完善可继承该组件)
 *     layout attributes
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_indicatorColor         获取页卡颜色值 (默认-0xffff00ff;)
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_dividerColor           获取分割线颜色的值 (默认-0xff000000)
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_bottomLineColor        获取底部线条颜色的值 (默认-0xff000000)
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_dividerMargin          获取分割线的距离上线边距的距离(默认-8)
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_indicatorHeight        获取页卡的高度
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_bottomLineHeight       获取底部线条的高度
 *     @attr-slidingTabLayout         ref R.styleable#SlidingTabLayout_dividerWidth           获取分割线的宽度
 * </pre>
...
public class SlidingTabLayout extends LinearLayout {
    ...
    public SlidingTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (attr == R.styleable.SlidingTabLayout_indicatorColor) {/*获取页卡颜色值*/
                mIndicatorColor = typedArray.getColor(attr, DEFAULT_INDICATOR_COLOR);

            } else if (attr == R.styleable.SlidingTabLayout_dividerColor) {/*获取分割线颜色的值*/
                mDividerColor = typedArray.getColor(attr, DEFAULT_DIVIDER_COLOR);

            } else if (attr == R.styleable.SlidingTabLayout_bottomLineColor) {/*获取底部线条颜色的值*/
                mBottomLineColor = typedArray.getColor(attr, DEFAULT_BOTTOM_LINE_COLOR);

            } else if (attr == R.styleable.SlidingTabLayout_dividerMargin) {/*获取分割线的距离上线边距的距离*/
...
//        }
    ...
    private void drawTextColor(int right) {
        TextView selects = (TextView) getChildAt(mSelectedPosition);
        int childwidth = mWmWidth/getChildCount();
//        if (right >= (childwidth*mSelectedPosition) + )
    }
    ...
    private void populateTabLayout() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (i == 0)
                textView.setTextColor(Color.rgb(0, 163, 238));
            else
                textView.setTextColor(Color.rgb(30, 29, 29));
    ...
    private class InternalViewPagerChange implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            for (int i = 0; i < getChildCount(); i++){
                ...
                if (i == position){
                    tv.setTextColor(Color.rgb(0, 163, 238));
                }else
                    tv.setTextColor(Color.rgb(30, 29, 29));
            }
